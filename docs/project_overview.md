...
このリポジトリは、ポケモン対戦のための **シミュレーション基盤** と **モンテカルロ木探索（MCTS）による対戦 AI**、および **ポケモンSV対応のダメージ計算 API** をまとめたプロジェクトです。
...
POSTGRES_HOST={} \
POSTGRES_PORT={} \
poetry run python scripts/matching.py
```
...
ポケモンSV対応のダメージ計算 API を立ち上げます。

```bash
poetry run uvicorn src.damage_calculator_api.main:app --reload
```
